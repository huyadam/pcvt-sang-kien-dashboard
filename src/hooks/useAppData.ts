...
// Tăng version khi cấu trúc dữ liệu thay đổi để xóa cache cũ
const CACHE_KEY = 'pcvt_sk_cache_v3';

// Xóa cache phiên bản cũ
['pcvt_sk_cache', 'pcvt_sk_cache_v2'].forEach(k => {
...
export function useAppData(user: User | null) {
  ...
  const normalizeApiData = (data: any): any => {
    ...
    if (normalized.tracking) {
      normalized.tracking = normalized.tracking
        ...
        .filter((t: any) =>
          t.ma_sk && VALID_STATUSES.includes(String(t.trang_thai || '').trim().toLowerCase())
        );
    }
    ...
    setMasterData(prev => {
      ...
      Object.values(next.departments).forEach(dept => {
        dept.items.forEach(item => {
          if (item.ma === maSk) item.trang_thai = newStatus;
        });
      });
      return next;
    });
  ...
  const handleSubmitScore = async (payload: ScorePayload) => {
    ...
    }).catch(() => toast.error('Lỗi kết nối khi gửi điểm', { id: toastId }));
  ...
  const handleSubmitTracking = async (payload: TrackingPayload) => {
    ...
    }).catch(() => toast.error('Lỗi kết nối khi cập nhật tiến độ', { id: toastId }));
  ...
  const handleDeleteScore = async (maSk: string) => {
    ...
    }).catch(() => toast.error('Lỗi kết nối khi xóa điểm', { id: toastId }));
